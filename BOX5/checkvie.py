    ...
    while True:
        ...
        for boite, last_time in last_heartbeats.items():
            if last_time is None or (now - last_time) > HEARTBEAT_TIMEOUT:
                print(f"[CHECKVIE BOX5] ⚠️ {boite} ne répond plus (dernier heartbeat à {last_time})")
                # Ici tu peux déclencher un signal d’anomalie ou action de survie
        time.sleep(5)
...
def listen_heartbeats():
    ...
    print(f"[CHECKVIE BOX5] Écoute les heartbeats sur {LISTEN_IP}:{LISTEN_PORT}")
    ...
    while True:
        try:
            ...
            if boite in last_heartbeats:
                ...
                print(f"[CHECKVIE BOX5] Reçu heartbeat de {boite}")
        except socket.timeout:
            pass  # Timeout pour permettre de checker les boîtes mortes
        except Exception as e:
            print(f"[CHECKVIE BOX5] Erreur: {e}")

if __name__ == "__main__":
            import threading
            import time

            print("[CHECKVIE BOX5] Démarrage dans 10 secondes pour laisser démarrer les autres boîtes...")
